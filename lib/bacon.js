...
// Single Statement instead of nested JSON.
bacon.is = 
bacon.has =
bacon.parser = 
bacon.regex =  
...
bacon.regex = {
  line: {
    ...
    entity: /^([_A-Za-z0-9-\*\@][a-zA-Z0-9_\>\#\.\s\[\]\=]*)$/,
    rule: /^([_A-Za-z0-9-][_A-Za-z0-9-]*)(\:|\=)?\s?(.*)/,
 ...
 */
bacon.cook = bacon.parser.cook = function(data) {
  ...
  for(i = 0; i < input.length; i++, line = input[i]) {
    if(line && line.length > 0) {
      ...
      if(bacon.is.variable(line)) {
        ...
        if (typeof output[s][k] === 'undefined'
        || Object.prototype.toString.call(output[s][k]) !== '[object Array]') output[s][k] = [];
        output[s][k].push(v);
  ...
  for(var x in output) {
    ...
    for(var y in output[x]) {
      if(!output[x].hasOwnProperty(y)) continue;
      for(var i = 0; i < output[x][y].length; i++) {
        source += '  ' + y + ': ' + output[x][y][i] + ';' + "\n";
      }
    }
